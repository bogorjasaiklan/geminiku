...
export const setupGithubCommand = {
    ...
    action: async (context) => {
        ...
        for (const workflow of workflows) {
            ...
            const curlCommand = buildCurlCommand(`https://raw.githubusercontent.com/google-github-actions/run-gemini-cli/refs/tags/${releaseTag}/examples/workflows/${workflow}`, [`--output "${gitRepoRoot}/.github/workflows/${fileName}"`]);
...
function buildCurlCommand(u, additionalArgs) {
    const args = [];
    args.push('--fail');
    args.push('--location');
