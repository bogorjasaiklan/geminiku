...
import { BaseTool, ToolConfirmationOutcome, Icon, ApprovalMode, } from '../index.js';
...
describe('CoreToolScheduler edit cancellation', () => {
    it('should preserve diff when an edit is cancelled', async () => {
        class MockEditTool extends BaseTool {
            constructor() {
                super('mockEditTool', 'mockEditTool', 'A mock edit tool', Icon.Pencil, {});
            }
            ...
            async execute(_params, _abortSignal) {
                return {
        ...
        // Cancel the edit
        const confirmationDetails = await mockEditTool.shouldConfirmExecute({}, abortController.signal);
        if (confirmationDetails) {
            await scheduler.handleConfirmationResponse('1', confirmationDetails.onConfirm, ToolConfirmationOutcome.Cancel, abortController.signal);
        }
        expect(onAllToolCallsComplete).toHaveBeenCalled();
        const completedCalls = onAllToolCallsComplete.mock
            .calls[0][0];
