...
import { populateMcpServerCommand, createTransport, isEnabled, discoverTools, discoverPrompts, hasValidTypes, } from './mcp-client.js';
...
describe('mcp-client', () => {
    ...
    describe('discoverTools', () => {
        ...
        it('should log an error if there is an error discovering a tool', async () => {
            ...
            vi.mocked(DiscoveredMCPTool).mockImplementation((_mcpCallableTool, _serverName, name) => {
                if (name === 'invalid tool name') {
                    throw testError;
                }
                return { name: 'validTool' };
            });
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith(`Error discovering tool: 'invalid tool name' from MCP server 'test-server': ${testError.message}`);
        });
