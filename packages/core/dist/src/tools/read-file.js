...
class ReadFileToolInvocation extends BaseToolInvocation {
    ...
    async execute() {
        const result = await processSingleFileContent(this.params.absolute_path, this.config.getTargetDir(), this.params.offset, this.params.limit);
        if (result.error) {
            return {
...
export class ReadFileTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(ReadFileTool.Name, 'ReadFile', `Reads and returns the content of a specified file. If the file is large, the content will be truncated. The tool's response will clearly indicate if truncation has occurred and will provide details on how to read more of the file using the 'offset' and 'limit' parameters. Handles text, images (PNG, JPG, GIF, WEBP, SVG, BMP), and PDF files. For text files, it can read specific line ranges.`, Icon.FileSearch, {
            properties: {
                absolute_path: {
