...
describe('ReadFileTool', () => {
    ...
    describe('build', () => {
        ...
        it('should throw error for relative path', () => {
            ...
            expect(() => tool.build(params)).toThrow(`File path must be absolute, but was relative: test.txt. You must provide an absolute path.`);
        });
        ...
        it('should throw error for non-positive limit', () => {
            ...
            expect(() => tool.build(paramsZero)).toThrow('Limit must be a positive number');
            const paramsNegative = {
                absolute_path: path.join(tempRootDir, 'test.txt'),
                offset: 0,
                limit: -5,
            };
            expect(() => tool.build(paramsNegative)).toThrow('Limit must be a positive number');
        });
        it('should throw error for schema validation failure (e.g. missing path)', () => {
            const params = { offset: 0 };
            expect(() => tool.build(params)).toThrow(`params must have required property 'absolute_path'`);
        });
    });
