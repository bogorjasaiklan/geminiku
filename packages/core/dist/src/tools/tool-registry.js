...
import { Icon, BaseTool } from './tools.js';
...
export class ToolRegistry {
    ...
    async discoverAllTools() {
        ...
        await discoverMcpTools(this.config.getMcpServers() ?? {}, this.config.getMcpServerCommand(), this, this.config.getPromptRegistry(), this.config.getDebugMode());
    }
    /**
     * Discovers tools from project (if available and configured).
     * Can be called multiple times to update discovered tools.
    ...
    async discoverToolsForServer(serverName) {
        ...
        if (serverConfig) {
            await discoverMcpTools({ [serverName]: serverConfig }, undefined, this, this.config.getPromptRegistry(), this.config.getDebugMode());
        }
    }
